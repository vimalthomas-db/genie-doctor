...
def deterministic_findings(m: dict) -> list:
    ...
    if bdef == 0 or runs == 0:
        steps.append({
            "category": "NO_BENCHMARK", "impact": "high", "source": DET,
            "step": "Define benchmark questions with ground-truth SQL and run an eval.",
            "why": "Accuracy is unproven without benchmarks.",
            "evidence": f"benchmark_questions_defined={bdef}, num_runs={runs}"})
    if txti == 0 and sqli == 0:
        steps.append({
            "category": "EMPTY_INSTRUCTIONS", "impact": "high", "source": DET,
            "step": "Add text instructions (domain, grain, key term definitions) and SQL examples.",
            "why": "No curation means Genie has no guidance to answer accurately.",
            "evidence": f"text_instructions={txti}, sql_instructions={sqli}"})
    elif txti == 0:
        steps.append({
            "category": "EMPTY_INSTRUCTIONS", "impact": "medium", "source": DET,
            "step": "Add text instructions defining key terms, grain, and scope.",
            "why": "SQL examples exist but there is no prose defining the domain terms.",
